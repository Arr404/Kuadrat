...
export const Container: FC<ContainerProps> = ({ snapToGrid }) => {
  ...
  const [EqBox, setEqBox] = useState<string>('');
  ...
  useEffect(() => {
    setEqBox(KoordinatToEquation(boxes))
  }, [boxes]);
          ...
          <DragDuplicateBox
            ...
            onClick={() =>
              addBox(
                ...
                boxesDuplicate[key].left,
                boxesDuplicate[key].top,
                boxesDuplicate[key].title
              )
            }
            {...(boxesDuplicate[key] as {
              top: number;
              left: number;
              title: string;
            })}
